...
| 仓库/社区技能修改界面 | 修改已发布技能或仓库技能的元数据（如显示名称、描述、标签等） |
| 技能历史版本管理界面 | 技能 Owner 查看历史版本列表、各个版本的审核状态及退回原因，支持撤回或重新提交 |
| 开发者中心/我的发布面板 | 集中管理用户自己创建或参与贡献的所有社区技能，查看累计下载、点赞数和反馈 |
| 审核日志查看器 | 允许开发者或管理员查看某一版本技能在流转中产生的完整审批备注和操作记录 |
