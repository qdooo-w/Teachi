...
2. 你获取的skill内容将会会有 `global-` / `user-` / `project-` 前缀。现规定架构如下：
    ...
    - 含有 `user-` 前缀的skill代表的是用户信息skill，存储的内容与用户个人信息、用户喜好、用户要求强相关，我们默认把其中的内容视作用户主体信息的参考内容，用于完成指定任务时进行参考并优化输出答案，因此你应该要学会每次在完成任务的时候进行结合。当用户要求更改，新建相关内容时，你可以提示用户这些文件夹；
