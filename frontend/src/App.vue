...
// ── 科目管理（重命名 / 删除） ────────────────────────────────────────────────
// openMenuKey / renamingKey 使用 'sidebar:project:{pid}' 命名空间以区分同 id 冲突
      ...
      <section class="relative flex min-w-0 flex-1 flex-col bg-[#f3f4f6] transition-all duration-300">
          ...
          <div class="flex min-w-0 items-center gap-3 overflow-hidden">
            ...
            <div class="flex items-center gap-2 truncate text-sm font-medium text-[#4b5563]">
              ...
              <template v-else-if="currentView === 'chat' && currentProject && currentSession">
                <span class="cursor-pointer text-[#9ca3af] hover:text-[#1f2937]" @click="router.push({ name: 'subject', params: { pid: $route.params.pid } })">{{ truncateText(currentProject.projectname, 8) }}</span>
                <span class="text-[#9ca3af]">/</span>
        ...
        <div class="relative flex-1 overflow-hidden">
          ...
          <RouterView v-slot="{ Component }">
            <component :is="Component" v-if="$route.name === 'overview' || $route.name === 'subject'" />
          </RouterView>
