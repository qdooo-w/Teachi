...
<template>
  <div class="space-y-6 text-center">
    ...
    <div>
      <h1 class="text-3xl font-semibold text-slate-900">验证邮件已发送</h1>
      <p class="mt-3 text-sm leading-7 text-slate-600">
        我们已向 <span class="font-semibold text-slate-900">{{ email || '您填写的邮箱' }}</span> 发送了一封验证邮件。
      </p>
    </div>
    ...
    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
      <button
        type="button"
        class="inline-flex justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
        @click="goToLogin"
      >
        返回登录
      </button>
